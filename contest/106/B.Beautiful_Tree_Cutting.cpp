...
0或5, 也需要做相同的计算并累加起来才能得到最终结果. 需要注意的是, 当s的第一个数字是0或者5时, 需要单独处理,
如果s的第一个数字是0或者5, 那么重复k次后得到的新数字串的第一个数字也是0或者5.
当第一个s[0]作为末尾数字时, 后面所有数字全部删掉, 有 1 种方案
当第二个s[0]作为末尾数字时, 其前面有n个数字, 所以有 2^n 种方案
当第三个s[0]作为末尾数字时, 其前面有2n个数字, 所以有 2^2n 种方案
当第k个s[0]作为末尾数字时, 其前面有(k-1)n个数字, 所以有 2^(k-1)n 种方案
所以总和为
1 + 2^n + 2^2n + ... + 2^(k-1)n
正好等于 tmp.
